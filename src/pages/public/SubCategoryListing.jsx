...
const SubCategoryListing = () => {
  ...
  // Dynamic URL logic maintained for compatibility
  const baseUrl = window.location.hostname === "localhost" 
    ? "http://localhost:5000" 
    : "https://event-planner-9dgd.onrender.com";

  // Dynamic content mapping based on reference image structure
  const listConfig = {
  ...
  useEffect(() => {
    const fetchVendorsBySubCategory = async () => {
      ...
      try {
        ...
        // This toast was appearing because of the catch block
        ...
        <div className="px-6 md:px-20 lg:px-32 py-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-gray-500 mt-2">{vendors.length} Verified Professionals Available</p>
            </div>
          </div>
          ...
          <div className="bg-[#111622] border border-white/5 p-4 rounded-2xl shadow-xl flex flex-1 max-w-md items-center gap-3 w-full">
            ...
            <input 
              placeholder={`Search verified ${serviceType}s...`}
